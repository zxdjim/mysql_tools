...
filters=('sr_fix_matchInfo                beginTime  0 90 0' 'sr_fix_matchInfo_history modifyTime  0 90 0' 'sr_live_matchStatus modifyTime 0 30 0'  
         'sr_live_eventStatisticsInfo_uof modifyTime 0 30 0' 'sr_live_eventInfo_bmk    createTime  0 7  7' 'sr_live_marketOdds  createTime 0 3  3'
...
filters=('sr_panda_fix_marketCategory modifyTime 0 30 0' 'sr_panda_fix_marketOutCome    modifyTime 0 30 0' 'sr_panda_fix_tournament               modifyTime 0 30 0'
         ...
         'sr_panda_live_eventInfo_bmk createTime 1  7 7' 'sr_panda_live_marketOdds      modifyTime 0 3  3' 'sr_panda_live_betSettlement           modifyTime 0 30 0'
                 'sr_panda_live_betStatus     modifyTime 0 30 0' 'sr_panda_live_marketCashOut   modifyTime 0 7  7')
del_mongo sr_panda "${filters[*]}"

## bg 库
filters=('bg_full_processmessage         modifyTime 0 3 0' 'bg_live_marketodds              CreateTime 1 7 0' 'bg_full_fixture                 CreateTime 1 30 0' 
...
## rb 库
filters=('rb_event_error createTimeStamp 0 3 0' 'rb_event_info createTimeStamp 0 3 0' 'rb_fix_matchinfo      createTimeStamp  0 3 0'
         'rb_message     createTimeStamp 0 3 0' 'rb_statistics createTimeStamp 0 3 0' 'replay_matchid_manager modifyTime      0 3 0')
del_mongo rb "${filters[*]}"

## filters=(集合名称 过滤字段名称 字段类型 当前集合保留的天数 备份保留的天数) 字段类型: 0-时间戳(timestamp) 1-时间字符串("YYYY-MM-DD HH:MI:SS")  

## rb_panda 库
filters=('rb_fix_matchinfo createTimeStamp 0 3 0' 'rh_match_event modifyTime 0 3 0' 'rh_match_info       modifyTime 0 3 0'
         'rh_match_status  modifyTime      0 3 0' 'rh_match_team  modifyTime 0 3 0' 'rh_match_tournament modifyTime 0 3 0')
del_mongo rb_panda "${filters[*]}"

## tx 库
filters=('tx_match matchTime 0 7 7' 'tx_all_odds matchTime 0 3 3' 'tx_odds matchTime 0 3 3')
del_mongo tx "${filters[*]}"

## tx_panda 库
filters=('rh_match_info   thirdMatchInfoDTO.beginTime 0 3 3' 'rh_match_market     modifyTime 0 3 3' 'multiple_ds_marketodds matchTime 0 3 3'
         'rh_match_status modifyTime                  0 3 3' 'rh_odds_field_cache matchTime  0 3 3' 'tx_match               matchTime 0 3 3')
del_mongo tx_panda "${filters[*]}"

## panda_report 库
filters=('standard_market_odds  modify_time    0 7 0' 'third_market_odds       modify_time    0 7  0'
         'third_market_odds_all dataSourceTime 0 3 3' 'third_market_status_all dataSourceTime 0 15 7')
del_mongo panda_report "${filters[*]}"

## ao_consumer 库
filters=('rh_match_market  modifyTime 0 7 7' 'rh_market_cashOut modifyTime 0 7 7')
del_mongo ao_consumer "${filters[*]}"

## ao_producer 库
filters=('reverse_apply_record createTime 0 15 15')
del_mongo ao_producer "${filters[*]}"


echo "/////////js结束处理删除过期数据/////////" >> ${js_path}

echo -e "\033[34;40m 开始删除过期数据...\033[0m"
echo "///////// 开始处���删除过期数据,当前时间: "`date +'%Y-%m-%d %T'` >> ${js_path}
