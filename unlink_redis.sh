...
do
   ...
   redis-cli -h ${v_ip} -p ${v_port} --no-auth-warning -a $token --scan --pattern "$delk" |xargs -i redis-cli -h ${v_ip} -p ${v_port} --no-auth-warning -a $token unlink {}
done
