...
###  v_list_type       必填项 列表类型 如 'P/A/NAL/AL/NATL/ATL'
    ...
    IF v_table_schema IS NULL OR v_table_name IS NULL OR v_list_type IS NULL 
		 ...
		 IF UPPER(TRIM(v_list_type)) NOT IN ('P','A','NAL','AL','NATL','ATL') then
			 RETURN '第三参数只可为P/A/NAL/AL/NATL/ATL 六种类型中的一种,请确认无误后再尝试!!!';
			 LEAVE label1;
    end if; 
...
CREATE PROCEDURE `sp_batch_dml`(v_dml_type char(1),
								...
								v_qry_where varchar(500),  v_qry_sub varchar(2000),  
								v_batch_like     bit,  v_batch_where varchar(1000), v_add_batch_where bit,
								v_batch_cnt int, v_sleep_sec decimal(4,2),
								v_debug varchar(5))
label1:BEGIN
### author:baider date:20211218
### 功能说明: 此过程小批量按指定的条件操作记录(第一参数指定:i/I:插入,u/U:更新,d/D:删除)
### DEMO: CALL my_tools.sp_batch_dml('i','my_tools','t_user','my_tools','t_user_bak','AL',null,'A','LAST_UPDATED_TIME < "2019-11-01"',null,1,null,0,10000,0.01,null);
### DEMO: CALL my_tools.sp_batch_dml('u','my_tools','t_user',null,null,null,'status=1','A','LAST_UPDATED_TIME < "2019-11-01"',null,1,null,0,10000,0.01,null);
### DEMO: CALL my_tools.sp_batch_dml('d','my_tools','t_user',null,null,null,null,'A','LAST_UPDATED_TIME < "2019-11-01"',null,1,null,0,10000,0.01,null);
### 带子查询的例子,前一参数用[]占位
### DEMO: CALL my_tools.sp_batch_dml('i','my_tools','t_rebuild_next_table_init','my_tools','t_diff','AL',null,'a','tid < 20 and tid in[]','select tid from t_rebuild_next_table_init',1,null,1,10000,0.01,'debug');
### DEMO: CALL my_tools.sp_batch_dml('u','my_tools','t_rebuild_next_table_init',null,null,null,'IS_ENABLE=1','A','tid < 20 and tid in[]','select tid from t_rebuild_next_table_init',1,null,1,10000,0.01,'debug');
### DEMO: CALL my_tools.sp_batch_dml('d','my_tools','t_rebuild_next_table_init',null,null,null,null,'A','tid < 20 and tid in[]','select tid from t_rebuild_next_table_init',1,null,1,10000,0.01,'debug');
##1 v_dml_type        必填项   需要进行操作的类型单一字符(i/I:插入,u/U:更新,d/D:删除)
##2 v_schema_source   必填项   需要操作的源库名
##3 v_table_source    必填项   需要操作的源表名
##4 v_schema_target   非必填项 插入(删除和更新无用)需要操作的目标库名
##5 v_table_target    非必填项 插入(删除和更新无用)需要操作的目标表名
##6 v_i_flist         非必填项 插入(删除和更新无用):用于源和目标表中字段列表,即insert into和SELECT后面的列表('AL','NAL'分别代表带种子列表和不带种子列表)
##7 v_u_flist         非必填项 更新(删除和插入无用):用于SET 后面的更新列表,请注意不要带【SET】关键字 
##8 v_field           必填项   需要用于批次中源表的过滤字段名('A','P',分别代表取本表的种子或主键,或者其他具体字段名)                    
##9 v_qry_where       非必填项 需要操作的源表的过滤条件(可为空,代表批量操作全表),请注意不要带【WHERE】关键字,如果有IN 从句的子查询请用[]占位,用v_qry_sub参数值替换之
#10 v_qry_sub         非必填项 (目前只支持一个子查询)用于源表中过滤条件后的子查询(没有则空),用临时表来保存中间结果集,用于替换v_qry_where中的[]占位符
#11 v_batch_like      必填项   用于控制v_qry_where与v_batch_where是否一样,因查询和最终批量操作的条件可能不一样,所以要增加2个参数
#12 v_batch_where     非必填项 v_batch_like=1,则用v_qry_where值,否则用本身的值用于批量操作
#13 v_add_batch_where 必填项   需要操作的源表的批量过程条件中是否还需要加此前一参数的条件 0:否 1:是
#14 v_batch_cnt       必填项   每一批次要处理的数量(根据具体情况,不可太大,建议:1W-10W)
#15 v_sleep_sec       必填项   批次间休眠时间秒数带2位小数,不可为空或0,不可太大,建议:10.00以内
#16 v_debug           非必填项 当输入debug(大小写均可)时,将是调试模式,只打印语句而不真正的执行
  ...
  if v_dml_type is null or length(trim(v_dml_type)) = 0 or (upper(v_dml_type)!='I' and upper(v_dml_type)!='U' and upper(v_dml_type)!='D') then
		SELECT CONCAT('ERROR:  (第1参数)操作类型必须为i/I/u/U/d/D之一且不可为空!!!') as result;
		leave label1;
	end if;	
  if v_schema_source is null or length(trim(v_schema_source)) = 0 or v_table_source is null or length(trim(v_table_source)) = 0 then
		SELECT CONCAT('ERROR:  (第2.3参数)源库名或源表名不可为空!!!') as result;
		leave label1;
	end if;
  ...
  if v_cnt = 0 then
		SELECT CONCAT('ERROR:  (第2.3参数)源库名或源表名不存在!!!') as result;
		leave label1;  
  end if;
  if upper(v_dml_type)='I' then	
	  if v_schema_target is null or length(trim(v_schema_target)) = 0 or v_table_target is null or length(trim(v_table_target)) = 0 then
			SELECT CONCAT('ERROR:  (第4.5参数)目标库名或目标表名不可为空!!!') as result;
			leave label1;
		end if;
	  ...
	  if v_cnt = 0 then
			SELECT CONCAT('ERROR:  (第4.5参数)目标库名或目标表名不存在!!!') as result;
			leave label1;
	  end if;
	  if v_i_flist is null or length(trim(v_i_flist)) = 0 then
			SELECT CONCAT('ERROR:  (第6参数)插入列表类型不可为空!!!') as result;
			leave label1;
		end if;
  end if;
  if upper(v_dml_type)='U' then	
	  if v_u_flist is null or length(trim(v_u_flist)) = 0 then
			SELECT CONCAT('ERROR:  (第7参数)更新列表类型不可为空!!!') as result;
			leave label1;
		end if;
  end if;
  if v_field is null or length(trim(v_field)) = 0 then
		SELECT CONCAT('ERROR:  (第8参数)依据的字段不可为空!!!') as result;
		leave label1;
	end if;	
  if v_qry_where is null or length(trim(v_qry_where)) = 0  then
        if length(trim(v_qry_sub)) > 0 or v_qry_sub is not null then
			SELECT CONCAT('温馨提示:  (第9参数)当前参数为空时,第10参数也必须为空!!!') as result;
			leave label1;		   
		end if;
		SELECT CONCAT('温馨提示:  (第9参数)过滤条件为空将操作此表所有数据(默认)!!!') as result;
		SET v_qry_where = ' ';
		SET v_len_where = 0;
	end if;
  if position('[]' in v_qry_where) > 0 and (v_qry_sub is null or length(trim(v_qry_sub)) = 0 or position(' from ' in v_qry_sub) = 0)  then
		SELECT CONCAT('温馨提示:  (第10参数)当第9参数含有[]占位符时,本参数必须不为空且必须含有from关键字的子查询!!!') as result;
        leave label1;
	end if;	
  if length(trim(v_qry_where)) > 0 and position('[]' in v_qry_where) = 0 and v_qry_sub is not null then
		SELECT CONCAT('温馨提示:  (第10参数)当第9参数不含有[]占位符时,本参数必须为空!!!') as result;
        leave label1;
	end if;	
  if v_batch_like+0 = 0 and position('[]' in v_batch_where) > 0 and (v_qry_sub is null or length(trim(v_qry_sub)) = 0 or position(' from ' in v_qry_sub) = 0)  then
		SELECT CONCAT('温馨提示:  (第10参数)当第11参数是0且第12参数含有[]占位符时,本参数必须不为空且必须含有from关键字的子查询!!!') as result;
        leave label1;
	end if;
 	
  if (v_batch_where is null or length(trim(v_batch_where)) = 0) and v_batch_like+0 = 0 then
		SELECT CONCAT('ERROR:  (第12参数)当第11参数为0时,此参数不可为空!!!') as result;
		leave label1;
	end if;	

  if v_batch_cnt is null or length(trim(v_batch_cnt)) = 0 or v_batch_cnt <= 0 then
		SELECT CONCAT('ERROR:  (第14参数)一批次的数量不可为空或非正数!!!') as result;
		leave label1;
	end if;	
  if v_sleep_sec is null or length(trim(v_sleep_sec)) = 0 or v_sleep_sec = 0 or v_sleep_sec = 0.00 then
		SELECT CONCAT('ERROR:  (第15参数)批次间休眠时间秒数带2位小数,不可为空或0!!!') as result;
		leave label1;
	end if;
begin
    ### 设置库名.表名变量
	...
	declare v_sql      varchar(500);  ## 保存v_qry_where 值或为空
    ...
    if v_cnt = 0 then
		SELECT CONCAT('ERROR:  (第8参数)依据的字段不存在!!!') as result;
		leave label1;
    end if;
	...
	ELSE
		...
		IF  length(trim(v_qry_sub)) > 0 then
			DROP TEMPORARY TABLE if exists my_tools.tmp_sub_qry;
			set @SQL=concat('CREATE TEMPORARY TABLE my_tools.tmp_sub_qry as(',replace(v_qry_sub,' from ',' as tmp_id from '),');');
			PREPARE STMT FROM @SQL;
			EXECUTE STMT;
			DEALLOCATE PREPARE STMT;
			create index idx_id on my_tools.tmp_sub_qry(tmp_id);
			IF position('[]' in v_qry_where) > 0 then
			   set v_qry_where = replace(v_qry_where,'[]','(select tmp_id from my_tools.tmp_sub_qry)');
            END IF;
			IF v_batch_like+0=0 and position('[]' in v_batch_where) > 0 then
			   set v_batch_where = replace(v_batch_where,'[]','(select tmp_id from my_tools.tmp_sub_qry)');
            END IF;            			
		END IF;

	    set @SQL = concat('SELECT MAX(',v_FID,'),MIN(',v_FID,') into @max_id, @min_id from ',v_schema_table_source,' WHERE ',trim(v_qry_where),';');
		IF v_batch_like+0=0 then
		   set v_sql = concat(' and ',trim(v_batch_where));
		ELSE
		   set v_sql = concat(' and ',trim(v_qry_where));
		END IF;
	END IF;
    ...
    if v_add_batch_where+0=0 or not v_add_batch_where then
       set v_sql = '';
	end if;
  ...
  SELECT CONCAT('开始',v_desc,' 【',v_schema_table_source,'】表中过滤条件为: ',trim(v_qry_where),' 的数据...') AS RESULT;
  if upper(trim(v_debug))='DEBUG' then
     select concat(v_dml_sql,' WHERE ',v_FID,' between ',v_cur_minID,' and ',v_cur_maxID,v_sql,';') as sql_result;
	  ...
	  while v_cur_minID <= @max_id do
            ...
            SELECT CONCAT('正在执行',v_desc,': ',@SQL) AS RESULT;			
			PREPARE STMT FROM @SQL;
			EXECUTE STMT;
			DEALLOCATE PREPARE STMT;
			commit;
  ...
  SELECT CONCAT('结束',v_desc,' 【',v_schema_table_source,'】表中过滤条件为: ',trim(v_qry_where),' 的数据!!!') AS RESULT;
...
CREATE TABLE  `t_move_data_init` (
  ...
  `add_batch_where`    bit(1) not null default b'1'  COMMENT '重要参数:在批次间是否需要再加上过滤参数, 0:不需要,1:需要(默认)',   
  ...
  `qry_where`          varchar(2000) NOT NULL COMMENT '用于源表中过滤条件(不需要加where关键字),用{}占位符,用后面时间替换;用[]占位符时,用后面子查询替换',
  `qry_sub`            varchar(2000) NULL default NULL COMMENT '用于源表中过滤条件后的子查询(没有则空),用临时表来保存中间结果集,可替换条件列的[]',
  `batch_like`         bit(1) NOT NULL default b'1'    COMMENT '用于控制batch_where与qry_where是否一样,因查询和最终批量操作的条件可能不一样',
  `batch_where`        varchar(2000) NULL default NULL COMMENT '当batch_like=1,则用qry_where值,否则用本身的值用于批量操作',  
...
INSERT INTO my_tools.t_move_data_init(is_bak,add_batch_where,source_schema,source_table,target_schema,target_table,field_name,qry_where,qry_sub,where_days,batch_cnt,sleep_sec)
...
### add by TY-baider  2021-06-24
		...
		declare v_add_batch_where  bit(1);        ## 在批次间是否需要再加上过滤参数, 0:不需要,1:需要(默认)
		...
		declare v_qry_where        varchar(2000); ## 用于源表中过滤条件(不需要加where关键字),用{}占位符,用后面时间替换;用[]占位符时,用后面子查询替换
		declare v_qry_sub          varchar(2000); ## 用于源表中过滤条件后的子查询(没有则空),用临时表来保存中间结果集,可替换条件列的[]
		declare v_batch_like       bit(1);        ## 用于控制v_batch_where与v_qry_where是否一样,因查询和最终批量操作的条件可能不一样
		declare v_batch_where      varchar(2000); ## 当v_batch_like=1,则用v_qry_where值,否则用本身的值用于批量操作		
		...
		DECLARE my_cur CURSOR for SELECT id,is_bak,add_batch_where,source_schema,source_table,target_schema,target_table,i_flist,i_flist,field_name,
		                                 qry_where,qry_sub,batch_like,batch_where,where_days,batch_cnt,sleep_sec,bak_status,del_status,debug
		                          FROM my_tools.t_move_data_init where is_enable = 1;
	  DECLARE CONTINUE HANDLER FOR NOT FOUND SET done = true;
		
	  open my_cur;
		read_loop:loop
	  fetch my_cur into  v_id,v_bak,v_add_batch_where,v_source_schema,v_source_table,v_target_schema,v_target_table,v_i_flist,v_u_flist,v_field_name,
	                     v_qry_where,v_qry_sub,v_batch_like,v_batch_where,v_where_days,v_batch_cnt,v_sleep_sec,v_bak_status,v_del_status,v_debug;
				IF done then                    
					leave read_loop;
				END IF;
				...
				IF v_bak+0=1 then
		            ...
		            set v_qry_where   = replace(v_qry_where,'{}',v_previous_date);
					set v_qry_sub     = replace(v_qry_sub,'{}',v_previous_date);
					set v_batch_where = replace(v_batch_where,'{}',v_previous_date);

					select '温馨提示: 休息3S后,开始插入数据...' result;
					select sleep(3);  ## 休眠一下				
					call my_tools.sp_batch_dml('i',v_source_schema, v_source_table, v_target_schema, v_target_table, v_i_flist, v_u_flist, v_field_name,
									           v_qry_where, v_qry_sub, v_batch_like+0, v_batch_where, v_add_batch_where+0, v_batch_cnt, v_sleep_sec, v_debug);
			    ...
			    call my_tools.sp_batch_dml('d',v_source_schema, v_source_table, v_target_schema, v_target_table, v_i_flist, v_u_flist, v_field_name, 
									       v_qry_where, v_qry_sub, v_batch_like+0, v_batch_where, v_add_batch_where+0, v_batch_cnt, v_sleep_sec, v_debug);				
...
STARTS concat(date_format(now(),'%Y-%m-%d'),' 08:00:00')
